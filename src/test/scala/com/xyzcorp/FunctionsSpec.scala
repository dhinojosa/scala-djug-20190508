...
import org.scalatest.{Entry, FunSuite, Matchers}
...
class FunctionsSpec extends FunSuite with Matchers {
  ...
  test(
    """Lab: Create a method called lessThan that accepts an integer, we'll call it x,
    ...
    val isFreezingCelcius: Int => Boolean = lessThan(0)
    val isFreezingFahreinheit: Int => Boolean = lessThan(32)

    isFreezingFahreinheit(40) should be(false)
  }
  ...
  test(
    ...
    result.get(100) should contain ("One Hundred")
    result.get(200) should contain ("Two Hundred")
    result.get(300) should contain ("Three Hundred")
  }
  ...
  test(
    """flatMap also wonderful for digging
    ...
    val result: Seq[Employee] = List(manager1, manager2)
      .flatMap(ma => ma.employees)
  }
  ...
  test(
    """foldRight will take two parameters group, the first
      |  will contain a seed and then a function that will
  ...
  test(
    ...
    val result = (1 to 10000000)
      .view //makes the rest lazy, like Java
