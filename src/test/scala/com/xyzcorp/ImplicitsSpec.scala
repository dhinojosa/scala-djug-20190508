...
class ImplicitsSpec extends FunSpec with Matchers {
  ...
  describe(
    ...
    it(
      ...
      def calcYearRate(amount: Int)
                      (implicit bonus: Int, currency: String): String = {
    ...
    it("""can also be replaced with default parameters, choose accordingly""") {
      ...
      def calcYearRate(amount: Int, bonus: Int = 1000,
                       currency: String = "Euro"): String = {
        amount + bonus + " " + currency
      }
    ...
    it(
      """If you have a List[String] implicitly will it try
    ...
    it(
      """Implicit wrappers can be created using a function
        |  and is often easier to mental map.""".stripMargin) {
    ...
    it(
      """Can be use a short hand version of this called implicit classes,
        |  before using them
    ...
    it(
      ...
      10.exclaim should be("10!")
      "Hello".exclaim should be("Hello!")
      List(1, 2, 3).exclaim should be("List(1, 2, 3)!")
    }
  ...
  describe("Locating implicits recipes") {
    ...
    it("""can also use a companion object to store any implicit recipes"""
        .stripMargin) {

      class Artist(val firstName: String, val lastName: String)
      object Artist {
        import scala.language.implicitConversions
        implicit def tuple2Artist(t: (String, String)): Artist = new Artist(
          t._1, t._2)
      }

      def playPerformer(a: Artist) = s"Playing performer ${a.firstName} ${
        a.lastName
      }"

      playPerformer("Weird Al" -> "Yankovic") should
        be("Playing performer Weird Al Yankovic")
    }

    it("""As an example we can use scala.collection.JavaConverters to convert the Java call,
  ...
  describe(
    ...
    it(
      ...
      writeItOut(new Employee("Carly", "Simon")) should
        be("Employee: Simon, Carly")
    }
  }
  ...
  describe(
    ...
    it(
      ...
      def equals[A](a: A, b: A)(implicit eqtc: Eq[A]): Boolean = eqtc
        .isEqual(a, b)
      ...
      equals(Team("Cincinnati", "Bengals"),
        Team("Cincinnati", "Bengals")) should be(true)
      equals2(Team("Cincinnati", "Bengals"),
        Team("Cincinnati", "Bengals")) should be(true)
    }

    it(
      """Ordering is a typeclass and is used to determine ordering.
